...
function CartDAO(database) {
    ...
    this.getCart = function(username, callback) {
        "use strict";
      ...
      var key = crypto.createCipher('aes256', username);
      var encryptedVal = key.update(username,'utf8','hex') +key.final('hex');



      var resultArray = database.find({"userId":encryptedVal}).toArray();
      resultArray.then(data =>{
        console.log(data);
        callback(data[0] === undefined?userCart:data[0]);
      })
      .catch(error =>{
        console.log(error);
      })
    ...
    this.itemInCart = function(userId, itemId, callback) {
         ...
         *
         */
         var itemToSendInCallback=null;
         var result=database.find({"userId":userId,"items._id":itemId}).toArray();
         result.then(data=>{
           var itemFound = (data == undefined)? null:data[0];
           console.log("in itemincart");
           console.log(itemFound);
           if(itemFound != null){
             itemToSendInCallback={};
             for(var k=0;k<itemFound.items.length;k++){
               if(itemFound.items[k]._id == itemId){
                 itemToSendInCallback = itemFound.items[k];
               }
             }
           }
           callback(itemToSendInCallback);
         }).catch(error=>{
           console.log(error);
         });
    ...
    this.updateQuantity = function(userId, itemId, quantity, callback) {
      ...
      /*  var userCart = {
       ...
       console.log(quantity);
        var resultOfUpdate;
        if(quantity == 0){
          resultOfUpdate = database.findOneAndUpdate(
            {
              "userId":userId
            },
            {
         ...
         resultOfUpdate.then(data=>{
           var result = (data == undefined) ?null :data;
console.log("result of in updateQuantity");
